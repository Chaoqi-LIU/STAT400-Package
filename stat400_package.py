...
class NegativeBinominal:
    ...
    def pdf(self, x): return Math.C(x - 1, self.r - 1) * (self.prob ** self.r) * ((1 - self.prob) ** (x - self.r))
...
class Binominal:
    ...
    def pdf(self, x): return Math.C(self.n, x) * (self.prob ** x) * ((1 - self.prob) ** (self.n - x))  
...
class Hypergeometric:
    ...
    def pdf(self, x): return Math.C(self.N1, x) * Math.C(self.N2, self.n - x) / Math.C(self.N, self.n)
    def cdf(self, x): return sum(self.pdf(i) for i in range(x + 1))
...
class Multinominal:
    ...
    def pdf(self, xs): return Math.fact(self.n) / math.prod(Math.fact(x) for x in xs) * math.prod(self.probs[i] ** xs[i] for i in range(self.k))
...
class Poisson:
    ...
    def pdf(self, x): return (self.lam ** x) * math.exp(-self.lam) / Math.fact(x)
    def cdf(self, x): return sum(self.pdf(i) for i in range(x + 1))
